# glow entity around
execute if entity @s[tag=mob_despawner] if score @s despawner.gticks matches 1.. run effect give @e[tag=!mob_despawner_base,distance=..96] glowing 2 0 true
...
###### creative ######
# despawn mobs
execute if entity @s[tag=mob_despawner_creative] run function laika:mob_despawner/events/despawning_entities_96
execute if entity @s[tag=bat_despawner] run execute as @e[type=bat,distance=..96] unless data entity @s CustomName run tp @s ~ ~-200 ~