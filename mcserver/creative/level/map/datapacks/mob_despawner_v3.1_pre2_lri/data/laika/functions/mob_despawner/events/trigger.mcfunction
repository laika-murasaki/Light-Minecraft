# check nearest
execute if entity @s[scores={despawner.trigg=256}] as @e[type=area_effect_cloud,tag=mob_despawner_base,distance=..256] at @s run function laika:mob_despawner/events/check_nearest
execute if entity @s[scores={despawner.trigg=128}] as @e[type=area_effect_cloud,tag=mob_despawner_base,distance=..64,limit=1,sort=nearest] at @s run function laika:mob_despawner/events/check_nearest
